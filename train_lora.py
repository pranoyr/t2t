...
def save_custom_checkpoint(accelerator, model, optimizer, lr_scheduler, tokenizer, global_step, checkpoint_dir):
    ...
    # PEFT's save_pretrained handles saving only the adapter weights (adapter_model.safetensors)
    if accelerator.is_main_process:
...
def load_custom_checkpoint(accelerator, model, optimizer, lr_scheduler, checkpoint_dir):
    ...
    if os.path.exists(safetensors_path):
        ...
        # set_peft_model_state_dict handles injecting the adapter weights
        ...
        accelerator.print(f"Warning: Could not find LoRA weights in {checkpoint_dir}")
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train Qwen Coder with LoRA on GPUMODE/KernelBook SFT dataset")
    ...
    parser.add_argument("--learning_rate", type=float, default=2e-4, help="Learning rate for LoRA (higher than full finetuning)")
    parser.add_argument("--num_epochs", type=int, default=3, help="Number of training epochs")
    parser.add_argument("--max_seq_length", type=int, default=4096, help="Max sequence length")
    parser.add_argument("--output_dir", type=str, default="./qwen-coder-lora", help="Directory to save checkpoints")
...
def main():
    ...
    # CHANGED: Cosine decay with warmup
